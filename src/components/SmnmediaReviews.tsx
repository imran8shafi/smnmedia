...
// ─── TESTIMONIAL SUB-COMPONENTS (each independently editable) ───────────────────────

/* PlayIcon – just the triangle SVG */
interface PlayIconProps extends React.SVGProps<SVGSVGElement> {
  width?: number;
  height?: number;
  fill?: string;
}
function PlayIcon({ width = 10, height = 12, fill = 'white', className = '', ...rest }: PlayIconProps) {
  return (
    <svg width={width} height={height} viewBox="0 0 8.4 9.6" fill={fill} className={className} {...rest}>
...
/* VerifiedBadge – blue starburst SVG */
interface VerifiedBadgeProps extends React.SVGProps<SVGSVGElement> {
  width?: number;
  height?: number;
  fill?: string;
}
function VerifiedBadge({ width = 8, height = 8, fill = '#4aadf4', className = '', ...rest }: VerifiedBadgeProps) {
  return (
    <svg width={width} height={height} viewBox="0 0 7.909 7.909" fill={fill} className={className} {...rest}>
...
/* PlayButtonOverlay – frosted circle + icon, absolutely positioned */
interface PlayButtonOverlayProps {
  className?: string;
  style?: React.CSSProperties;
  buttonClassName?: string;
  buttonStyle?: React.CSSProperties;
  iconProps?: PlayIconProps;
}
function PlayButtonOverlay({
  ...
  return (
    <div className={`absolute inset-0 flex items-center justify-center ${className}`} style={style}>
      <div
        className={`w-9 h-9 rounded-full flex items-center justify-center ${buttonClassName}`}
        style={{ backdropFilter: 'blur(5px)', backgroundColor: 'rgba(255,255,255,0.12)', ...buttonStyle }}
      >
        <PlayIcon {...iconProps} />
      </div>
    </div>
...
/* TestimonialVideo – video + optional play overlay */
interface TestimonialVideoProps {
  ...
  overlayProps?: PlayButtonOverlayProps;
}
const TestimonialVideo = forwardRef<HTMLVideoElement, TestimonialVideoProps>(function TestimonialVideo(
  {
    src,
    poster,
    playing = false,
  ...
  return (
    <div
      ...
      style={{ width: 200, height: 220, borderRadius: 12, overflow: 'hidden', ...style }}
      onClick={onToggle}
    >
      <video
        ref={ref}
        ...
        style={{ borderRadius: 12, ...videoStyle }}
      />
      {!playing && <PlayButtonOverlay {...overlayProps} />}
    </div>
  );
});

/* TestimonialText – quote + highlight + trailing text */
...
function TestimonialText({
  ...
  style,
  highlightClassName = 'mark-highlight',
  highlightStyle,
  afterClassName = '',
  afterStyle,
  ...
  return (
    ...
    >
      {text}{' '}
      {highlight && (
        <mark className={highlightClassName} style={highlightStyle}>
          {highlight}
        </mark>
      )}
      {textAfter && (
        <span className={afterClassName} style={afterStyle}>
          {' '}{textAfter}
        </span>
      )}
    </p>
...
/* TestimonialAuthor – avatar + name + badge + clinic */
interface TestimonialAuthorProps {
  ...
  clinicStyle?: React.CSSProperties;
  badgeProps?: VerifiedBadgeProps;
}
function TestimonialAuthor({
  ...
  style,
  avatarClassName = 'w-6 h-6 rounded object-cover',
  avatarStyle,
  nameClassName = 'text-xs font-semibold',
  nameStyle,
  clinicClassName = 'text-xs opacity-40',
  clinicStyle,
  badgeProps = {},
}: TestimonialAuthorProps) {
  return (
    <div className={className} style={style}>
      {avatar && <img src={avatar} alt={name} className={avatarClassName} style={avatarStyle} />}
      <div>
        <div className="flex items-center gap-1">
          <span
            className={nameClassName}
            style={{ fontFamily: 'Hanken Grotesk, sans-serif', ...nameStyle }}
          >
            {name}
          </span>
          {verified && <VerifiedBadge {...badgeProps} />}
        </div>
        {clinic && (
          <p
            className={clinicClassName}
            style={{ fontFamily: 'Hanken Grotesk, sans-serif', ...clinicStyle }}
          >
            {clinic}
          </p>
        )}
...
/* TestimonialContent – right-hand text column */
interface TestimonialContentProps {
  testimonial: Testimonial;
  className?: string;
  style?: React.CSSProperties;
  textProps?: Partial<TestimonialTextProps>;
  authorProps?: Partial<TestimonialAuthorProps>;
}
function TestimonialContent({
  ...
  return (
    <div className={className} style={{ minHeight: 140, ...style }}>
      <TestimonialText
        text={testimonial.text}
        highlight={testimonial.highlight}
        textAfter={testimonial.textAfter}
        {...textProps}
      />
      <TestimonialAuthor
...
/* TestimonialCard – top-level composition */
interface TestimonialCardProps {
  testimonial: Testimonial;
  className?: string;
  style?: React.CSSProperties;
  videoProps?: Partial<TestimonialVideoProps>;
  contentProps?: Partial<TestimonialContentProps>;
}
function TestimonialCard({
  ...
  return (
    <div className={className} style={{ width: 380, borderRadius: 12, ...style }}>
      <TestimonialVideo
        ref={videoRef}
        ...
        {...videoProps}
      />
      <TestimonialContent testimonial={testimonial} {...contentProps} />
    </div>
  );
}
